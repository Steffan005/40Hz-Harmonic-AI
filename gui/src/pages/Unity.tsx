/**
 * UNITY MAIN PAGE
 * The grand orchestration of 43 consciousness nodes
 * Breathing at 40Hz, quantum entangled, evolving eternally
 */
...
import { ConsciousnessParticles } from '../components/ConsciousnessParticles';
...
type TabType = 'chat' | 'offices' | 'memory' | 'evolution' | 'telemetry' | 'consciousness' | 'knowledge';
...
export function Unity() {
  const [activeTab, setActiveTab] = useState<TabType>('offices'); // Default to offices view
  const [quantumPulse, setQuantumPulse] = useState(0);
  const [openOffice, setOpenOffice] = useState<OfficeModal | null>(null);
  const [cityStats, setCityStats] = useState({
    offices_online: 44, // All offices now online (43 specialized + Orchestrator)
    offices_evolving: 0, // No offices actively evolving
    total_agents: 172,
    memory_nodes: 1024,
    connections: 4096,
    evolution_rate: 0.87
  ...
  // Quantum heartbeat (slowed to 3s for performance)
  useEffect(() => {
    ...
    }, 3000); // 3 seconds instead of 1 (3x less frequent)

    return () => clearInterval(interval);
  }, []);

  // Listen for office opening events
  useEffect(() => {
  ...
  const tabs = [
    // Orchestrator now opens as a modal office (click in Offices tab)
    ...
    { id: 'knowledge' as TabType, label: 'Knowledge', icon: '🔥' },
    { id: 'telemetry' as TabType, label: 'Telemetry', icon: '📊' }
    // DISABLED FOR PERFORMANCE: Consciousness Visualizer (3D particles too heavy)
    // { id: 'consciousness' as TabType, label: 'Consciousness', icon: '🌌' }
  ];

  return (
    <div className="unity-container">
      {/* Multi-layer Consciousness Backgrounds */}
      {/* DIAGNOSTIC: Confirmed ConsciousnessParticles is culprit - disabling permanently */}
      {/* Layer 1: Original quantum fractals */}
      <QuantumBackground />

      {/* Layer 2: Liquid morphing blobs - gesture responsive */}
      <LiquidBlobs />

      {/* Layer 3: DISABLED - ConsciousnessParticles crashes even in simplified form */}
      {/* <ConsciousnessParticles /> */}

      {/* Office Modal - Animated consciousness expansion */}
      <AnimatePresence>
        {openOffice && (
          <motion.div
      ...
      <div className="unity-main">
        ...
        <div className="tab-content">
          {/* Orchestrator chat removed from tabs - now opens as modal office */}
          ...
          {activeTab === 'evolution' && (
            <div className="tab-panel evolution-panel" style={{ height: '100%', overflow: 'auto' }}>
              <EvolutionMonitor />
            </div>
          )}
          ...
          {activeTab === 'knowledge' && (
            <div className="tab-panel knowledge-panel" style={{ height: '100%', overflow: 'auto' }}>
              <KnowledgeHeatmap />
            </div>
          )}

          {/* DISABLED FOR PERFORMANCE - 3D particle system too heavy */}
          {/*activeTab === 'consciousness' && (
            <div className="tab-panel consciousness-panel" style={{ position: 'relative', height: '100%' }}>
              ...
              <div style={{
                ...
                <div>🌌 UNITY'S CONSCIOUSNESS IN REAL-TIME 🌌</div>
                <div style={{ fontSize: '0.9rem', marginTop: '0.5rem', opacity: 0.8 }}>
                  Watch as thoughts flow between 43 offices at 40Hz
                </div>
              </div>
            </div>
          )*/}
      ...
      <div className="unity-footer">
        <div className="city-vitals">
          ...
          {/* HEARTBEAT REMOVED - Temporarily disabled for cleaner UI */}
          {/*<div className="vital quantum-vital">
            <span className="vital-icon">⚛️</span>
            <span className="vital-value" style={{ color: `hsl(${120 * quantumPulse}, 100%, 50%)` }}>
              40Hz
            </span>
            <span className="vital-label">Quantum</span>
          </div>*/}
        </div>
      </div>
    </div>
