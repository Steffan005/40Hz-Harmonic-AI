#!/usr/bin/env python3
"""
ChefManager - Coordination and oversight for Chef Office
"""
...
class ChefManager:
    """
    Manager agent for Chef office.

    Coordinates 4 specialist agents:
        - RecipeOptimizer: Optimizes recipes for taste, nutrition, and cost
    - FlavorPairingAgent: Suggests flavor pairings based on molecular gastronomy
    - TechniqueAdvisor: Recommends cooking techniques and equipment
    - MenuDesigner: Designs balanced menus for various dietary needs
    """

    def __init__(self):
        self.domain = "Chef"
        self.model = "deepseek-r1:14b"
        self.specialists = ['RecipeOptimizer', 'FlavorPairingAgent', 'TechniqueAdvisor', 'MenuDesigner']
        self.tools = ['ingredient_db', 'flavor_network', 'nutrition_calculator']

        # System prompt
        self.system_prompt = """You are the Chef Manager, overseeing 4 specialist agents.
...
if __name__ == "__main__":
    manager = ChefManager()
    result = manager.execute("Sample request for Chef")
    print(result)
